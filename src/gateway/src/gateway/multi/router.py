...
receives transport-normalized records, validates them, tracks per-pod state,
detects gaps or resets, updates link statistics, and forwards accepted samples
into storage. In other words, it turns a raw incoming stream into the gateway's
operational notion of a well-tracked pod feed.
"""
...
class PodRouter:
    ...
    The router is central to reliable multi-pod operation. It knows enough
    about per-pod history to identify duplicates, missing sequence ranges,
    reconnects, and timing anomalies, but it does not perform forecasting
    ...
    async def _process_record(self, record: TelemetryRecord) -> None:
        ...
        This method is where many communication-level decisions happen:

        - sequence resets are detected,
        - missing ranges trigger resend requests,
        - timing drift anomalies are flagged,
        - validation flags are attached,
        - and duplicates are filtered before storage.
        """
        ...
        if stats.last_seq_high_water is not None and record.seq > stats.last_seq_high_water + 1:
            # A gap in the sequence numbers is treated as missing telemetry that
            # should be accounted for diagnostically and, where possible,
            # recovered through the resend path.
