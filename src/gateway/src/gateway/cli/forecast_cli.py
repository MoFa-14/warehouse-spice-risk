...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    run.add_argument("--every-minutes", type=int, default=5, help="Forecast cadence in minutes. Defaults to 5.")
...
def _add_common_arguments(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    parser.add_argument("--data-root", help="Optional data-root override for raw telemetry, forecasts, and case storage.")
...
def cli(argv: Sequence[str] | None = None) -> int:
    ...
    lock = GatewayProcessLock(build_lock_path(runner.lock_target))
    try:
        lock.acquire()
